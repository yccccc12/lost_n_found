...
import { backendItemToItemEntry, recordDetailHref, type BackendItem } from '@/lib/backend-items'
      ...
      const rows = await Promise.all(
        results.map(async (ai) => {
          try {
            ...
            return { entry: backendItemToItemEntry(raw), ai }
          } catch {
            return { entry: aiOnlyEntry(ai), ai }
          }
        }),
            ...
            <ul className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {aiRows.map(({ entry, ai }) => (
                <li key={ai.item_id} className="flex flex-col gap-3">
                  ...
                  <ItemCard item={entry} href={recordDetailHref(entry.id)} />
                  <p
                    className={cn(
